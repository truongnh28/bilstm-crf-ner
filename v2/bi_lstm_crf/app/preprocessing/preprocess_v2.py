...
from v2.bi_lstm_crf.app.preprocessing import save_json_file, load_json_file, PAD, OOV
...
def __build_corpus_for_test(file_path):
    xs, ys = [], []
    with open(file_path, encoding="utf8") as f:
        for idx, line in tqdm(enumerate(f), desc="parsing {}".format(file_path)):
            fields = line.strip().split("\t")
...
if __name__ == '__main__':
    preprocessor = Preprocessor(config_dir='../sample_corpus', save_config_dir='.', verbose=True)
    x_test, y_test = __build_corpus_for_test('../sample_corpus/dataset_test.txt')
    print(x_test)
    # print(preprocessor)
    # print(x_test)